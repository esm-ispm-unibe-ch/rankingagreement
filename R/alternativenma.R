...
alternativenma <- function(netmetaobject,random=T,small.values="good"){
  ...
  if (small.values=="good") {Pscoreaverage=round(1-pnorm(TE/seTE), digits = 5)         #change from original altnma script: swapped with one from "if" below to match Pscore equation in paper
                             TE_ranks=rank(TE)}
  if (small.values=="bad") {Pscoreaverage=round(pnorm(TE/seTE), digits = 5)
